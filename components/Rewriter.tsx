// components/Rewriter.tsx
import React, { useState, useCallback, useEffect } from 'react';
import { generateTextContent } from '../services/geminiService';
import { DEFAULT_AI_MODEL, SYSTEM_INSTRUCTION_REWRITER } from '../constants';
...
export const Rewriter: React.FC<AIActionProps> = ({ initialInput, onActionComplete, onLogDebug, onSpeakText }) => {
  ...
  const rewriteText = useCallback(async (textToRewrite: string) => {
    setIsLoading(true);
    setError(null);
    try {
      if (!process.env.API_KEY) {
        ...
        onLogDebug?.({
          id: `rewriter-api-key-missing-${Date.now()}`,
          timestamp: new Date(),
          level: DebugLogLevel.API_KEY_MISSING,
          message: "Rewriter failed: API Key is not configured.",
        });
        return;
      }
      ...
      if (err.message.includes("API Key is not configured")) { // Check for the custom error message
        ...
        setError("API Key is not configured. Please ensure your API key is set in the environment.");
        onLogDebug?.({
          id: `rewriter-api-key-invalid-${Date.now()}`,
          timestamp: new Date(),
          level: DebugLogLevel.API_KEY_MISSING,
          message: `Rewriter failed: API Key invalid.`,
          details: { error: err.message },
        });
      } else {
        setError(`Failed to rewrite: ${err.message}`);
        onLogDebug?.({
          id: `rewriter-error-${Date.now()}`,
          timestamp: new Date(),
          level: DebugLogLevel.ERROR,
          message: `Rewriter failed: ${err.message}`,
          details: { error: err.message, prompt: textToRewrite },
        });
      }
    } finally {
      setIsLoading(false);
  ...
  const handleSpeakClick = () => {
    if (rewrittenText && onSpeakText) {
      onSpeakText(rewrittenText);
    }
  };
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      "Rewrite text in a clear and fluent way, avoiding plagiarism while preserving meaning and tone."
    ),
    React.createElement(
      "div",
      null,
      React.createElement(
        "label",
        { htmlFor: "rewriter-input", className: "block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1" },
        "Input Text:"
      ),
