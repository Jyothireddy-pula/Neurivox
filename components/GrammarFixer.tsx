// components/GrammarFixer.tsx
import React, { useState, useEffect, useCallback } from 'react';
import { generateTextContent } from '../services/geminiService';
import { DEFAULT_AI_MODEL, SYSTEM_INSTRUCTION_GRAMMAR_FIXER } from '../constants';
...
export const GrammarFixer: React.FC<AIActionProps> = ({ initialInput, onActionComplete, onLogDebug, onSpeakText }) => {
  ...
  const fixGrammar = useCallback(async (textToFix: string) => {
    setIsLoading(true);
    setError(null);
    try {
      if (!process.env.API_KEY) {
        ...
        onLogDebug?.({
          id: `grammar-api-key-missing-${Date.now()}`,
          timestamp: new Date(),
          level: DebugLogLevel.API_KEY_MISSING,
          message: "Grammar Fixer failed: API Key is not configured.",
        });
        return;
      }
      ...
      if (err.message.includes("API Key is not configured")) { // Check for the custom error message
        ...
        setError("API Key is not configured. Please ensure your API key is set in the environment.");
        onLogDebug?.({
          id: `grammar-api-key-invalid-${Date.now()}`,
          timestamp: new Date(),
          level: DebugLogLevel.API_KEY_MISSING,
          message: `Grammar Fixer failed: API Key invalid.`,
          details: { error: err.message },
        });
      } else {
        setError(`Failed to fix grammar: ${err.message}`);
        onLogDebug?.({
          id: `grammar-error-${Date.now()}`,
          timestamp: new Date(),
          level: DebugLogLevel.ERROR,
          message: `Grammar Fixer failed: ${err.message}`,
          details: { error: err.message, prompt: textToFix },
        });
      }
    } finally {
      setIsLoading(false);
  ...
  const handleSpeakClick = () => {
    if (fixedText && onSpeakText) {
      onSpeakText(fixedText);
    }
  };
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      "Correct grammar, spelling, and punctuation. Maintain original tone and style."
    ),
    React.createElement(
      "div",
      null,
      React.createElement(
        "label",
        { htmlFor: "grammar-input", className: "block text-sm font-medium text-gray-700 mb-1" },
        "Input Text:"
      ),
      React.createElement("textarea", {
        ...
        placeholder: "Enter text here to fix its grammar...",
        value: inputText,
